...
function PostCard({post, className}) {
    return (
        ...
        <div className={`w-full rounded-2xl border-2 border-b-black  m-2 p-2 bg-white ${className}`}>
            <Link to={`/post/${post.$id}`}>
                    <img className="w-full rounded-2xl h-32" src={Service.getFilePreview(post.featured_image)} alt={post.title}/>
                    <h2>{post.title}</h2>
            </Link>
        </div>
    )
